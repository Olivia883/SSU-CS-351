...
int main(int argc, char* argv[]) {
   ...
   // Wait for the last thread explicitly (optional, std::jthread auto-joins, but safe)
   threads.back().join();
   ...
   for (size_t i = 0; i < numThreads; ++i) {
    ...
    // Compute and print the volume estimate
	std::cout << "Estimated volume fraction = " 
        << static_cast<double>(volumePoints) / numSamples << "\n";
}
